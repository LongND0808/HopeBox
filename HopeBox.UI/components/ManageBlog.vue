        ...
        <table class="blog-table">
          ...
          <tbody>
            <tr v-for="blog in blogs" :key="blog.id">
              ...
              <td data-label="Ngày đăng">{{ formatDate(blog.createdDate) }}</td>
        ...
        <div class="modal">
          ...
          <div class="form-group">
            ...
            <select v-model.number="form.status" required>
              <option v-for="(label, value) in BlogStatusLabel" :key="value" :value="Number(value)">
                {{ label }}
              </option>
            </select>
          </div>
          <div class="modal-actions">
            <button @click="saveBlog" class="btn btn-success" :disabled="!form.title || !form.content || !form.status.toString()">Lưu</button>
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (response.data.status === 200) {
          this.blogs = response.data.responseData.map(blog => ({
            ...blog,
            imageUrl: blog.bannerImage || '/images/placeholder.jpg'
          }));
        } else {
    ...
    getStatusLabel(status) {
      return BlogStatusLabel[status] || 'Không xác định';
    },
    ...
    editBlog(blog) {
      const createdDate = blog.createdDate ? new Date(blog.createdDate).toISOString().slice(0, 16) : '';
      this.form = {
        id: blog.id || '',
        title: blog.title || '',
        content: blog.content || '',
        status: Number(blog.status) || BlogStatus.DRAFT,
        imageUrl: blog.imageUrl || '',
        createdDate: createdDate,
        viewCount: blog.viewCount || 0,
        createdBy: blog.createdBy || '',
        organizationId: blog.organizationId || '',
        createdByName: blog.createdByName || null,
        organizationName: blog.organizationName || null
      };
    ...
    async handleImageUpload(event) {
      ...
      try {
        ...
        const blogId = this.form.id || this.editingBlog?.id || '';
    ...
    async saveBlog() {
      if (!this.form.title || !this.form.content || !this.form.status.toString()) {
        await showErrorAlertDark('Lỗi', 'Vui lòng nhập đầy đủ thông tin bắt buộc: Tiêu đề, Nội dung, Trạng thái.');
        return;
      }
      ...
      try {
        const payload = {
          ...
          createdDate: this.form.createdDate ? new Date(this.form.createdDate).toISOString() : null,
          viewCount: this.form.viewCount || 0,
          createdBy: this.form.createdBy || null,
          organizationId: this.form.organizationId || null,
          createdByName: this.form.createdByName || null,
          organizationName: this.form.organizationName || null
        };

        const url = this.editingBlog
          ? `${BASE_URL}/api/blog/update`
          : `${BASE_URL}/api/blog/add`;
        ...
        if ([200, 201].includes(response.data.status)) {
          await showSuccessAlertDark('Thành công', this.editingBlog ? 'Cập nhật blog thành công!' : 'Thêm blog thành công!');
          this.fetchBlogs();
          this.closeModal();
        } else {
