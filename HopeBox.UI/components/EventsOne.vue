<template>
    <section class="events-area events-default-area">
        <div class="container">
            <div class="row">
                <div class="col-lg-8">
                    <div class="section-title" data-aos="fade-up" data-aos-duration="1000">
                        <h5 class="subtitle line-theme-color mb-7">Sự kiện mới nhất</h5>
                        <h2 class="title title-style">
                            Tham gia các sự kiện gây quỹ mới cùng HopeBox
                            <img class="img-shape" src="/images/shape/3.png" alt="Image">
                        </h2>
                    </div>
                    <div class="events-content events-list" data-aos="fade-up" data-aos-duration="1000">
                        <div class="event-item">
                            <div class="thumb">
                                <img class="thumb-img" src="/images/events/1.jpg" alt="Image">
                                <nuxt-link to="/event-details" class="btn-theme btn-gradient btn-size-sm">
                                    Tham gia ngay
                                    <img class="icon icon-img" src="/images/icons/arrow-line-right.png" alt="Icon">
                                </nuxt-link>
                            </div>
                            <div class="content">
                                <div class="event-info">15 Tháng 1, 2021  // <span>Giáo dục</span></div>
                                <h4 class="event-name">
                                    <nuxt-link to="/event-details">Xây lớp học cho trẻ em vùng cao Hà Giang</nuxt-link>
                                </h4>
                            </div>
                        </div>
                        <div class="event-item">
                            <div class="thumb">
                                <img class="thumb-img" src="/images/events/2.jpg" alt="Image">
                                <nuxt-link to="/event-details" class="btn-theme btn-gradient btn-size-sm">
                                    Tham gia ngay
                                    <img class="icon icon-img" src="/images/icons/arrow-line-right.png" alt="Icon">
                                </nuxt-link>
                            </div>
                            <div class="content">
                                <div class="event-info">23 Tháng 2, 2021  // <span>Sức khỏe</span></div>
                                <h4 class="event-name">
                                    <nuxt-link to="/event-details">Khám bệnh miễn phí cho trẻ em Lào Cai</nuxt-link>
                                </h4>
                            </div>
                        </div>
                        <div class="event-item">
                            <div class="thumb">
                                <img class="thumb-img" src="/images/events/3.jpg" alt="Image">
                                <nuxt-link to="/event-details" class="btn-theme btn-gradient btn-size-sm">
                                    Tham gia ngay
                                    <img class="icon icon-img" src="/images/icons/arrow-line-right.png" alt="Icon">
                                </nuxt-link>
                            </div>
                            <div class="content">
                                <div class="event-info">10 Tháng 3, 2021  // <span>Hỗ trợ mùa đông</span></div>
                                <h4 class="event-name">
                                    <nuxt-link to="/event-details">Tặng áo ấm cho trẻ em Mù Cang Chải</nuxt-link>
                                </h4>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="col-8 offset-2 col-sm-8 offset-sm-2 col-md-8 offset-md-2 col-lg-4 offset-lg-0">
                    <div class="layer-style">
                        <div class="thumb tilt-animation">
                            <img src="/images/photos/event1.png" alt="Image">
                            <div class="play-video-btn">
                                <a class="btn-play play-video-popup wave-btn" href="https://www.youtube.com/watch?v=eS9Qm4AOOBY">
                                    <span></span>
                                    <span></span>
                                    <span></span>
                                    <div class="icon">
                                        <img src="/images/icons/play.png" alt="Icon">
                                    </div>
                                </a>
                            </div>
                        </div>
                        <div class="shape-style1">
                            <img src="/images/shape/line1.png" alt="Image">
                        </div>
                        <div class="shape-style2">
                            <img src="/images/shape/line2.png" alt="Image">
                        </div>
                        <div class="shape-style3">
                            <img src="/images/shape/line3.png" alt="Image">
                        </div>
                        <div class="shape-style4">
                            <img src="/images/shape/line4.png" alt="Image">
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
</template>
